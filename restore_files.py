#!/usr/bin/env python3
"""
Dashboard File Reconstructor
Replaces corrupted .ts/.tsx files with clean, production-ready versions
"""
...
# Clean server.ts content
SERVER_TS_CONTENT = '''import express from 'express';
...
def main():
    print("╔═══════════════════════════════════════════════════════════════╗")
    print("║  DASHBOARD FILE RECONSTRUCTOR                               ║")
    print("║  Replacing corrupted files with clean, production-ready code ║")
    print("╚═══════════════════════════════════════════════════════════════╝\\n")
    
    files_to_restore = [
        ("server.ts", SERVER_TS_CONTENT),
    ]
    
    for filename, content in files_to_restore:
        filepath = Path(DASHBOARD_PATH) / filename
        
        # Backup original
        backup_path = Path(DASHBOARD_PATH) / f"{filename}.corrupted"
        if filepath.exists():
            shutil.copy(filepath, backup_path)
            print(f"📦 Backed up: {filename} → {filename}.corrupted")
        
        # Write clean version
        filepath.write_text(content, encoding='utf-8')
        print(f"✅ Restored: {filename}")
    
    print("\\n🎉 Dashboard files restored successfully!")
    print("\\n📋 Next steps:")
    print("   1. cd c:\\\\Users\\\\regan\\\\ID SYSTEM\\\\axiom-x\\\\constitutional-market-harmonics\\\\dashboard")
    print("   2. npm install")
    print("   3. npm run build")
    print("   4. npx tsx server.ts (Terminal 1)")
    print("   5. npm run dev (Terminal 2)")
